...
def main():
    ...
    model_tmp = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=test_set)

    if model_tmp.__class__.__name__ == 'CenterPoint':
        model, cfg = prune_cp.pruning(cfg, model_tmp, test_set)
    else:
        model, cfg = prune_plain_model.pruning(cfg, model_tmp, test_set)
