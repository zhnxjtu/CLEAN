...
def train_eval_single_ckpt_with_teacher(model, teacher_model, test_loader, args, eval_output_dir, logger, epoch_id, ckpt_dir, dist_test=False):
    # load checkpoint
    filename = os.path.join(ckpt_dir, 'checkpoint_epoch_%s.pth' % epoch_id)
    model.load_params_from_file(filename=filename, logger=logger, to_cpu=dist_test)
    model.cuda()
    ...
    # start evaluation
    eval_utils.eval_one_epoch_with_tea(
        cfg, args, model, teacher_model, test_loader, epoch_id, logger, dist_test=dist_test,
        result_dir=eval_output_dir, save_to_file=False
    )
