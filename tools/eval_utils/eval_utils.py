...
def eval_one_epoch_with_tea(cfg, args, model, teacher_model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False,
                   result_dir=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            import copy
            batch_dict_save = batch_dict.copy()
            pred_dicts_tea, _ = teacher_model(batch_dict)

            batch_dict = batch_dict_save.copy()
            batch_dict['pred_dicts_tea'] = pred_dicts_tea
            pred_dicts, ret_dict = model(batch_dict)
            # batch_dict.pop('pred_dicts_tea')
        disp_dict = {}
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            if model.module.__class__.__name__ == 'CenterPoint':
                ...
                mask_pruning = generate_dict['mask_pruning'].unsqueeze(-1) # [16, 35200, 1]
            else:
                ...
                mask_pruning = generate_dict['mask_pruning'].unsqueeze(-1) 
                ...
                if key.split('.')[0] in ['backbone_2d', 'backbone_3d']:
                    if params.dim() == 5:
                        ...
                        for f_count in range(c_out):
                            ...
                            # TODO: masked capr
                            err_tmp = ((torch.sigmoid(pred_cur) - torch.sigmoid(pred_tmp)).abs().squeeze() * mask_pruning).sum(-1).sum(-1)
                            err_tmp = err_tmp / torch.clamp(mask_pruning.squeeze().sum(-1), min=1)
                            err = err_tmp.sum()
                            # TODO: capr
...
def eval_one_epoch_measurement_resnet(cfg, args, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False,
                   result_dir=None, criterion_file_dir=None, dataset_name=None):
    result_dir.mkdir(parents=True, exist_ok=True)
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            for key, params in model_tmp.named_parameters():
                ...
                if key.split('.')[0] in ['backbone_2d', 'backbone_3d']:
                    if params.dim() == 5:
                        if (len(key.split('.')) == 5) and (key.split('.')[3] == 'conv1'):
                            c_out, h1, h2, h3, c_in = params.shape
                            for f_count in range(c_out):
                                para_save = params[f_count, ...].clone()
                                params[f_count, ...] = 0.0
