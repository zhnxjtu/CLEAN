...
class SECONDNet(Detector3DTemplate):
    ...
    def forward(self, batch_dict, record_time=None, **kwargs):
        ...
        if self.training:
            ...
            for cur_module in self.module_list:
                # if cur_module.__class__.__name__ == 'KDPointTrans' or \
                #         cur_module.__class__.__name__ == 'VoxelProbeHead':
                if cur_module.__class__.__name__ == 'KDPointTrans' or str(cur_module)[0:12] == 'KDPointTrans':
                    continue
                if record_time:
                    end = time.time()
                batch_dict = cur_module(batch_dict)
    ...
    def get_training_loss(self):
        ...
        for cur_module in self.module_list:
            if cur_module.__class__.__name__ == 'VoxelProbeHead':
                loss_probe, tb_dict = self.probe_block.get_loss(tb_dict)
                loss = loss + loss_probe
