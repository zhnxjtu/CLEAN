...
class Detector3DTemplate(nn.Module):
    ...
    def build_probe_block(self, model_info_dict):
        if self.model_cfg.get('PROBE_BLOCK', None) is None:
            return None, model_info_dict
        point_head_module = probe.__all__[self.model_cfg.PROBE_BLOCK.NAME](
            model_cfg=self.model_cfg.PROBE_BLOCK,
            input_channels=model_info_dict['num_point_features'],
            backbone_channels=model_info_dict['backbone_channels'],
            point_cloud_range=model_info_dict['point_cloud_range'],
            voxel_size=model_info_dict['voxel_size'],
            num_class=self.num_class if not self.model_cfg.PROBE_BLOCK.CLASS_AGNOSTIC else 1,
        )
    ...
    def build_dense_head(self, model_info_dict):
        ...
        #     grid_size=model_info_dict['grid_size'],
        #     point_cloud_range=model_info_dict['point_cloud_range'],
        #     predict_boxes_when_training=self.model_cfg.get('ROI_HEAD', False) or
        #                                 self.model_cfg.get('KD_TRANS_BLOCK', False) or
        #                                 self.model_cfg.get('PROBE_BLOCK', False),
        #     voxel_size=model_info_dict.get('voxel_size', False)
        # )
    ...
    # TODO: add by zhanghn, 2024-5-3
    def post_processing_stage_one(self, batch_dict):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        if batch_dict['batch_cls_preds_tea_stage1'].shape[-1] == 1:
            batch_dict['cls_preds_vina'] = batch_dict['batch_cls_preds_tea_stage1']

        for index in range(batch_size):
            if batch_dict.get('batch_index', None) is not None:
                assert batch_dict['batch_box_preds_tea_stage1'].shape.__len__() == 2
                batch_mask = (batch_dict['batch_index'] == index)
            else:
                assert batch_dict['batch_box_preds_tea_stage1'].shape.__len__() == 3
                batch_mask = index

            box_preds = batch_dict['batch_box_preds_tea_stage1'][batch_mask]
            ...
            # iou, iou_idx = iou3d_rcnn.max(dim=1)


            # print(box_preds.shape)
            # exit()                

            if not isinstance(batch_dict['batch_cls_preds_tea_stage1'], list):
                cls_preds = batch_dict['batch_cls_preds_tea_stage1'][batch_mask]
                ...
                cls_preds = [x[batch_mask] for x in batch_dict['batch_cls_preds_tea_stage1']]
                src_cls_preds = cls_preds
                if not batch_dict['cls_preds_normalized']:
    ...
    def post_processing(self, batch_dict):
        ...
        recall_dict = {}
        pred_dicts = []
        # print(self.model_cfg.LABEL_ASSIGN_KD.USE_STAGE_ONE)
        # exit()

        mask_pruning = torch.zeros(batch_dict['batch_cls_preds'].shape[:2]).cuda()
        if batch_dict['batch_cls_preds'].shape[-1] == 1:
            batch_dict['cls_preds_vina'] = batch_dict['batch_cls_preds']
        ...
        for index in range(batch_size):
            ...
            # if 'pred_dicts_tea' in batch_dict:
            #         cur_tea_box = batch_dict['pred_dicts_tea'][index]['pred_boxes']
            #         cur_tea_score = batch_dict['pred_dicts_tea'][index]['pred_scores']
            ...
            #             k -= 1
            #         cur_gt = cur_gt[:k + 1]
            #         cur_tea_score = cur_tea_score[:k + 1]
            #         cur_tea_score_mask = cur_tea_score >= 0.3
            #         cur_gt = cur_gt[cur_tea_score_mask]

            #         if cur_tea_score_mask.sum() > 0:
            #             final_iou = iou3d_nms_utils.boxes_iou3d_gpu(box_preds[:, 0:7], cur_gt[:, 0:7])
            #             # final_iou = final_iou.max(-1)[0]
            #         else:
            #             final_iou = torch.ones(box_preds[:, 0:7].shape[0],1).cuda()
            ...
            if batch_dict.get('batch_iou_preds', None) is not None:
                ...
                # iou_preds = batch_dict['batch_iou_preds'][batch_mask]
                # iou_preds = iou_preds.view(-1, 2)
                # iou_preds = iou_preds.argmax(dim=-1)
                # # print(iou_preds.shape)
                # print('aaaa',iou_preds.sum())
                # # exit()



            # iou_preds_gt = (iou>0.5).float()
            # print(iou_preds_gt.sum())
            # exit()
                ...
                if not batch_dict['cls_preds_normalized']:
                    if batch_dict.get('batch_iou_preds', None) is not None:
                        ...
                        if post_process_cfg.get('SOFT_REFINE', None) and post_process_cfg.SOFT_REFINE:
                            cls_preds = torch.sigmoid(cls_preds).pow(0.5) * iou_preds.repeat(1, 3).pow(0.5)
                            # cls_preds = torch.sigmoid(cls_preds)
                        else:
                            threshold = torch.ones_like(cls_preds)
                            threshold *= torch.tensor(post_process_cfg.POST_IOU_THRESH).cuda()
                            cls_preds = torch.sigmoid(cls_preds) * (iou_preds.repeat(1, 3) > threshold).float()
                            # cls_preds = torch.sigmoid(cls_preds)
                            # cls_preds = torch.sigmoid(cls_preds).pow(0.5) * iou_preds.repeat(1, 3).pow(0.5)


                        # iou_label = torch.sigmoid(cls_preds).argmax(-1)
                        # iou_preds_generate = torch.zeros_like(cls_preds)
                        # iou_preds_generate[torch.arange(iou_label.shape[0]), iou_label] = iou_preds[torch.arange(iou_label.shape[0]), 0]
                        # threshold = torch.ones_like(cls_preds)
                        # threshold *= torch.tensor(post_process_cfg.POST_IOU_THRESH).cuda()
                        # cls_preds = torch.sigmoid(cls_preds) * (iou_preds_generate > threshold).float()


                        # cls_preds = torch.sigmoid(cls_preds) * (iou_preds.max(-1)[0].unsqueeze(-1) > post_process_cfg.POST_IOU_THRESH).float()
                        # # TODO: zhanghn 2024-4-22
                        # cls_preds = torch.sigmoid(cls_preds) * iou_preds.unsqueeze(-1)
                        # cls_preds = torch.sigmoid(cls_preds)
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                # label_preds_tmp =label_preds[selected]
                # print('aa---', cls_preds[selected][label_preds_tmp==1].mean())
                # print('bb---', cls_preds[selected][label_preds_tmp==2].mean())
                # print('cc---', cls_preds[selected][label_preds_tmp==3].mean())
                ...
                # iou_preds = iou_preds[selected,0]
                # iou_select = (iou_preds>(iou_preds.mean()-iou_preds.std())).nonzero().squeeze()
                # print(iou_select)
                # print(selected)
                # exit()
                ...
                if self.is_teacher:
                    final_scores_before_nms = cls_preds
                    final_labels_before_nms = label_preds
                    final_boxes_before_nms = box_preds

                # print('bbbbbbbbbbbbbbb', selected.shape)
                # print(iou_preds.sum())
                # print(final_scores.shape)
                # exit()
                # if iou_select.dim() != 0:
                #     final_scores = final_scores[iou_select]
                #     final_labels = final_labels[iou_select]
                #     final_boxes = final_boxes[iou_select]
                ...
                #     # print(cur_tea_box.shape)
                #     # print(batch_dict['gt_boxes'][index].shape)
                #     # exit()
                ...
                #     if cur_tea_score_mask.sum() > 0:
                #         final_iou = iou3d_nms_utils.boxes_iou3d_gpu(final_boxes[:, 0:7], cur_gt[:, 0:7])
                #         final_iou = final_iou.max(-1)[0]
                #         # print('a', iou_preds)
                #         # print('b', final_iou)
                #         # exit()
                #         final_scores = final_scores[final_iou>0.3]
                #         final_labels = final_labels[final_iou>0.3]
                #         final_boxes = final_boxes[final_iou>0.3]

                mask_pruning[index][selected] = 1
            ...
            if self.is_teacher and 'LABEL_ASSIGN_KD' in self.model_cfg and self.model_cfg.LABEL_ASSIGN_KD.USE_BEFORE_NMS:
                record_dict = {
                    'pred_boxes': final_boxes,
                    'pred_scores': final_scores,
                    'pred_labels': final_labels,
                    'pred_scores_before_nms': final_scores_before_nms,
                    'pred_labels_before_nms': final_labels_before_nms,
                    'pred_boxes_before_nms': final_boxes_before_nms
                }
            else:
                record_dict = {
                    'pred_boxes': final_boxes,
        ...
        batch_dict['mask_pruning'] = mask_pruning
