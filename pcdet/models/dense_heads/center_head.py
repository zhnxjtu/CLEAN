...
class CenterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            if self.model_cfg.get('NUM_IOU_CONV', None):
                ...
                cur_head_dict['iou'] = dict(out_channels=1, num_conv=self.model_cfg.NUM_IOU_CONV)
    ...
    def assign_target_of_single_head(
        ...
        iou = gt_boxes.new_zeros(1, feature_map_size[1], feature_map_size[0])  # [3,200,176]
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            # a = (heatmap[cur_class_id]>0).nonzero().squeeze()
            # print(heatmap[cur_class_id][a[:,0], a[:,1]])
            # print(a.shape)
            # print(k)
            # exit()
            ...
            iou[0, center_int[k, 1], center_int[k, 0]] = 1
    ...
    # TODO: add by zhanghn, 2024-6-4
    def assign_target_of_single_head_weight(
            self, num_classes, gt_boxes, feature_map_size, feature_map_stride, weight_from_teacher, num_max_objs=500,
            gaussian_overlap=0.1, min_radius=2, hm_filter_num=None
    ):
        """
        Args:
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):
            ...
            # iou[cur_class_id, center_int[k, 1], center_int[k, 0]] = 1
            # iou[0, center_int[k, 1], center_int[k, 0]] = weight_from_teacher[k]
            if weight_from_teacher is None:
                 iou[0, center_int[k, 1], center_int[k, 0]] = 1.0
            else:
                iou[0, center_int[k, 1], center_int[k, 0]] = (1**self.model_cfg.TARGET_ASSIGNER_CONFIG.IOU_PERCENTAGE) * weight_from_teacher[k].pow(1.0-self.model_cfg.TARGET_ASSIGNER_CONFIG.IOU_PERCENTAGE)
        
        return heatmap, ret_boxes, inds, mask, iou
    
    # TODO: add by zhanghn, 2024-6-4
    def assign_targets_weight(self, gt_boxes, weight_anchor_list, feature_map_size=None, hm_filter_num_list=None, **kwargs):
        """
        Args:
            gt_boxes: (B, M, 8)
            feature_map_size: (2) [H, W]
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            for bs_idx in range(batch_size):
                ...
                # print('eeee', cur_gt_boxes.shape)
                # print('ffff', weight_anchor_list[bs_idx].shape)
                cur_weight = weight_anchor_list[bs_idx]
                ...
                heatmap, ret_boxes, inds, mask, iou = self.assign_target_of_single_head_weight(
                    num_classes=len(cur_class_names), gt_boxes=gt_boxes_single_head.cpu(),
                    feature_map_size=feature_map_size, feature_map_stride=target_assigner_cfg.FEATURE_MAP_STRIDE,
                    weight_from_teacher = cur_weight,
                    num_max_objs=target_assigner_cfg.NUM_MAX_OBJS,
                    gaussian_overlap=target_assigner_cfg.GAUSSIAN_OVERLAP,
                    min_radius=target_assigner_cfg.MIN_RADIUS,
                    hm_filter_num=None if hm_filter_num_list is None else hm_filter_num_list[bs_idx]
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts, no_nms=False, nms_config=None):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            if self.model_cfg.get('NUM_IOU_CONV', None) and not self.training:
                if post_process_cfg.get('SOFT_REFINE', None) and post_process_cfg.SOFT_REFINE:
                    # batch_hm = batch_hm.pow(0.5) * pred_dict['iou'].sigmoid().repeat(1, batch_hm.shape[1], 1, 1).pow(0.5)
                    # batch_hm = batch_hm * pred_dict['iou'].sigmoid().repeat(1, batch_hm.shape[1], 1, 1).pow(post_process_cfg.IOU_PERCENT)
                    batch_hm = batch_hm.pow(post_process_cfg.CLS_PERCENT) * pred_dict['iou'].sigmoid().repeat(1, batch_hm.shape[1], 1, 1).pow(1-post_process_cfg.CLS_PERCENT)
                else:
                    threshold = torch.ones(*batch_hm.shape[1:]).cuda()
                    threshold *= torch.tensor(post_process_cfg.POST_IOU_THRESH).unsqueeze(-1).unsqueeze(-1).cuda()
                    threshold = threshold.unsqueeze(0).repeat(batch_size, 1, 1, 1)
                    batch_iou = pred_dict['iou'].sigmoid().repeat(1, batch_hm.shape[1], 1, 1)
                    batch_hm = batch_hm * (batch_iou > threshold).float()
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                ...
                if post_process_cfg.NMS_CONFIG.NMS_TYPE != 'circle_nms' and not no_nms:
                    # print(final_dict['pred_scores'].max())
                    # print(final_dict['pred_scores'].min())
                    # exit()
                    ...
                    # TODO: add in 2024-4-28, zhanghn
                    final_dict['pred_inds'] = final_dict['pred_inds'][selected]
                    # print(final_dict['pred_boxes'].shape)
                    # print('aaaa', selected_scores)
                    # print('bbbb', batch_hm[0].max(0)[0].view(-1)[final_dict['pred_inds']])
                    # exit()
                ...
                # TODO: add in 2024-4-28, zhanghn
                ret_dict[k]['pred_inds'].append(final_dict['pred_inds'])
        ...
        for k in range(batch_size):
            ...
            # TODO: add in 2024-4-28, zhanghn
            ret_dict[k]['pred_inds'] = torch.cat(ret_dict[k]['pred_inds'], dim=0)

        return ret_dict
    
    def generate_predicted_boxes_mix(self, batch_size, pred_dicts, gt, no_nms=False, nms_config=None):
        post_process_cfg = self.model_cfg.POST_PROCESSING
        post_center_limit_range = torch.tensor(post_process_cfg.POST_CENTER_LIMIT_RANGE).cuda().float()
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                ...
                if post_process_cfg.NMS_CONFIG.NMS_TYPE != 'circle_nms' and not no_nms:
                    ...
                    # exit()
                    cur_gt = gt[k]
                    kkk = cur_gt.__len__() - 1
                    while kkk > 0 and cur_gt[kkk].sum() == 0:
                        kkk -= 1
                    cur_gt = cur_gt[:kkk + 1]
                    iou3d_rcnn = iou3d_nms_utils.boxes_iou3d_gpu(final_dict['pred_boxes'][:, 0:7], cur_gt[:, 0:7])
                    iou, _ = iou3d_rcnn.max(dim=1)
                    selected, selected_scores = model_nms_utils.class_agnostic_nms(
                        box_scores=final_dict['pred_scores'].pow(0.5) * iou.pow(0.5), box_preds=final_dict['pred_boxes'],
                        nms_config=post_process_cfg.NMS_CONFIG if nms_config is None else nms_config,
                        score_thresh=None
                    )
                    # print('a', selected.sort()[0])

                    final_dict['pred_boxes'] = final_dict['pred_boxes'][selected]
                    # final_dict['pred_scores'] = selected_scores
                    final_dict['pred_scores'] = final_dict['pred_scores'][selected]
                    final_dict['pred_labels'] = final_dict['pred_labels'][selected]
                    # TODO: add in 2024-4-28, zhanghn
                    final_dict['pred_inds'] = final_dict['pred_inds'][selected]
                    # print(final_dict['pred_boxes'].shape)
    ...
    def forward(self, data_dict):
        ...
        if self.training or self.model_cfg.get('FORCE_ASSIGN_TARGET', None):
            ...
            if self.kd_head is not None and not self.is_teacher and self.model_cfg.get('LABEL_ASSIGN_KD', None):
                ...
                # )
                if self.model_cfg.LABEL_ASSIGN_KD.get('USE_STAGE_ONE') and self.model_cfg.LABEL_ASSIGN_KD.USE_STAGE_ONE:   # use stage 1 for label assignment
                    ...
                    target_boxes, num_target_boxes_list, weight_anchor_list = self.kd_head.parse_teacher_pred_to_targets(
                            kd_cfg=self.model_cfg.LABEL_ASSIGN_KD, pred_boxes_tea=data_dict['decoded_pred_tea'],
                            gt_boxes=target_boxes, cost_thre = None
                        )
                    # print(num_target_boxes_list)

            if  'NAME' not in self.model_cfg.TARGET_ASSIGNER_CONFIG or self.model_cfg.TARGET_ASSIGNER_CONFIG.NAME == 'AxisAlignedTargetAssigner':
                ...
                target_dict = self.assign_targets_weight(
                    target_boxes, weight_anchor_list=weight_anchor_list,
                    feature_map_size=pred_dicts[0]['hm'].size()[2:],
                    feature_map_stride=data_dict.get('spatial_features_2d_strides', None),
                    hm_filter_num_list=hm_filter_num_list, 
                )
        ...
        if self.is_teacher:
            data_dict['pred_dicts_tea'] = pred_dicts
        ...
        if (not self.training and not self.is_teacher) or self.predict_boxes_when_training:
            ...
            if self.is_teacher and self.model_cfg.POST_PROCESSING.get('USE_MIX', False):
                pred_dicts = self.generate_predicted_boxes_mix(
                    data_dict['batch_size'], pred_dicts, data_dict['gt_boxes']
                )
            else:
                pred_dicts = self.generate_predicted_boxes(
                    data_dict['batch_size'], pred_dicts
            ...
            # TODO: add in 2024-4-28, zhanghn
            batch_size, _, hm_h, hm_w = pred_dicts_tmp[0]['hm'].shape
            mask_pruning = torch.zeros((batch_size, hm_h*hm_w)).cuda()
            for bs in range(batch_size):
                cur_inds = pred_dicts[bs]['pred_inds']
                mask_pruning[bs][cur_inds] = 1
                # print(len(cur_inds))
            # print(len(pred_dicts_tmp))
            # print(mask_pruning.sum(1))
            # exit()
            # print(self.is_teacher)
            # print(self.predict_boxes_when_training)
            ...
            if self.is_teacher:
                data_dict['final_box_dicts_tea_stage1'] = pred_dicts
            ...
            if self.predict_boxes_when_training or (not self.training and self.model_cfg.get('PRED_BOX_WHEN_EVAL', None)):
                ...
                data_dict['mask_pruning'] = mask_pruning
