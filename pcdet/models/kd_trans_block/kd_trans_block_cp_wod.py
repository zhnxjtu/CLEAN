...
class KDPointTrans_cp_wod(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        if self.model_cfg.get('MODE'):
            if self.model_cfg.MODE == 'TKD':
                self.trans_mode = 'TKD'
            elif self.model_cfg.MODE == 'DKD':
                self.trans_mode = 'DKD'
        else:
            self.trans_mode = 'SKD'
    ...
    def proposal_layer_cp2anchor(self, batch_dict, nms_config):
        # post_process_cfg = nms_config.POST_CONFIG
        post_center_limit_range = torch.tensor(self.point_cloud_range).cuda().float()
        batch_size = batch_dict['batch_size']

        # tea knowledge from center head
        pred_dicts_tea = batch_dict['pred_dicts_tea']

        # stu knowledge from anchor head
        ...
        mask = batch_box_preds_stu.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE, 1))
        

        # if self.model_cfg.get('USE_REG', None) and self.model_cfg.USE_REG:
        #     box_preds_stu_ori = dense_reg_preds_stu.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE,
        #                                               batch_box_preds_stu.shape[-1]))
  

        top_inds = torch.zeros((batch_size, nms_config.NMS_POST_MAXSIZE), dtype=torch.int64).cuda()
        top_class_ids = batch_box_preds_stu.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE))
        top_ys = batch_box_preds_stu.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE))
        top_xs = batch_box_preds_stu.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE))
        _, _, height, width = pred_dicts_tea[0]['hm'].size()
        ...
        for index in range(batch_size):
            ...
            if nms_config.MULTI_CLASSES_NMS:
                ...
                #     print(u)
                # exit()
                # print('a', len(selected_minor))

                top_inds[index, :len(selected_minor)] = selected_minor // 6
                # top_inds[index, :len(selected_minor)] = (selected_minor % (height * width))
                top_class_ids[index, :len(selected_minor)] = cur_roi_labels_stu[selected_minor]
                top_ys[index, :len(selected_minor)] = ((selected_minor // 6) // width).float()
                top_xs[index, :len(selected_minor)] = ((selected_minor // 6) % width).int().float()

            # ''' stu-to-tea '''
            cur_cls_preds = dense_cls_preds_stu[batch_mask]
            cls_select_reverse[index, :len(selected_minor), :] = cur_cls_preds[selected_minor]
            mask[index, :len(selected_minor), :] = 1.0
            # if self.model_cfg.get('USE_REG', None) and self.model_cfg.USE_REG:
            #     box_preds_stu_ori[index, :len(selected_minor), :] = box_preds_stu_ori_tmp[selected_minor]
        
        # TODO: student knowledege
        batch_dict['cls_preds_selected_reverse'] = cls_select_reverse
        # if self.model_cfg.get('USE_REG', None) and self.model_cfg.USE_REG:
        #     batch_dict['box_preds_stu_ori'] = box_preds_stu_ori


        for idx, _ in enumerate(pred_dicts_tea):

            if idx > 0:
                assert False # currently support one head
            # 
            rois_reverse = batch_box_preds_stu.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE, batch_box_preds_stu.shape[-1]))
        
            center = pred_dicts_tea[idx]['center']
            center_z = pred_dicts_tea[idx]['center_z']
            dim = pred_dicts_tea[idx]['dim'].exp()
            rot_cos = pred_dicts_tea[idx]['rot'][:, 0].unsqueeze(dim=1)
            rot_sin = pred_dicts_tea[idx]['rot'][:, 1].unsqueeze(dim=1)
            vel = pred_dicts_tea[idx]['vel'] if 'vel' in self.model_cfg.HEAD_ORDER else None

            # print(center.shape)
            # print(top_inds.shape)
            # exit()

            K = self.model_cfg.NMS_CONFIG.NMS_POST_MAXSIZE
            # print(top_inds)
            center = self._transpose_and_gather_feat(center, top_inds).view(batch_size, K, 2)
            rot_sin = self._transpose_and_gather_feat(rot_sin, top_inds).view(batch_size, K, 1)
            rot_cos = self._transpose_and_gather_feat(rot_cos, top_inds).view(batch_size, K, 1)
            center_z = self._transpose_and_gather_feat(center_z, top_inds).view(batch_size, K, 1)
            dim = self._transpose_and_gather_feat(dim, top_inds).view(batch_size, K, 3)
            angle = torch.atan2(rot_sin, rot_cos)
            top_xs = top_xs.view(batch_size, K, 1) + center[:, :, 0:1]
            top_ys = top_ys.view(batch_size, K, 1) + center[:, :, 1:2]
  
            point_cloud_range=self.point_cloud_range
            voxel_size=self.voxel_size
            feature_map_stride = self.model_cfg.FEATURE_MAP_STRIDE
            xs = top_xs * feature_map_stride * voxel_size[0] + point_cloud_range[0]
            ys = top_ys * feature_map_stride * voxel_size[1] + point_cloud_range[1]

            box_part_list = [xs, ys, center_z, dim, angle]
            if vel is not None:
                vel = self._transpose_and_gather_feat(vel, top_inds).view(batch_size, K, 2)
                box_part_list.append(vel)
            ...
            assert post_center_limit_range is not None
            mask_tmp = (final_box_preds[..., :3] >= post_center_limit_range[:3]).all(2)
            mask_tmp &= (final_box_preds[..., :3] <= post_center_limit_range[3:]).all(2)

            # if post_process_cfg.SCORE_THRESH is not None:
            #     mask_tmp &= (final_scores > post_process_cfg.SCORE_THRESH)
            ...
            for k in range(batch_size):
                ...
                with torch.no_grad():
                    # print(mask[k, :, 0].sum())
                    mask[k, :, 0] = cur_mask * mask[k, :, 0]
                    # print(mask[k, :, 0].sum())
                    rois_reverse[k, cur_mask, :] = final_box_preds[k, cur_mask]
                    # print('b', cur_mask.sum())


        # print(rois_reverse.shape)
        # exit()
        batch_dict['rois_reverse'] = rois_reverse
        batch_dict['mask_use_item'] = mask
    ...
    def proposal_layer_cp_tkd(self, batch_dict, nms_config):
        ...
        for idx, pred_dict in enumerate(pred_dicts_tea):
            ...
            # TODO: TEACHER DECODE
            batch_hm_tea = pred_dict['hm'].sigmoid()
            batch_center_tea = pred_dict['center']
            batch_center_z_tea = pred_dict['center_z']
            batch_dim_tea = pred_dict['dim'].exp()
            batch_rot_cos_tea = pred_dict['rot'][:, 0].unsqueeze(dim=1)
            batch_rot_sin_tea = pred_dict['rot'][:, 1].unsqueeze(dim=1)
            batch_vel_tea = pred_dict['vel'] if 'vel' in post_process_cfg.HEAD_ORDER else None
            ...
            final_pred_dicts = self.decode_bbox_from_heatmap(
                heatmap=batch_hm_tea, rot_cos=batch_rot_cos_tea, rot_sin=batch_rot_sin_tea,
                center=batch_center_tea, center_z=batch_center_z_tea, dim=batch_dim_tea, vel=batch_vel_tea,
                point_cloud_range=self.point_cloud_range, voxel_size=self.voxel_size,
                feature_map_stride=post_process_cfg.FEATURE_MAP_STRIDE,
                K=post_process_cfg.MAX_OBJ_PER_SAMPLE,
                circle_nms=(post_process_cfg.NMS_TYPE == 'circle_nms'),
            ...
            rois_reverse =final_pred_dicts[0]['pred_boxes'].new_zeros((batch_size, nms_config.TRAIN.NMS_POST_MAXSIZE, final_pred_dicts[0]['pred_boxes'].shape[-1]))
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                ...
                with torch.no_grad():
                    ...
                    mask[k, :len(selected), :] = 1.0
                    rois_reverse[k, :len(selected), :] = final_pred_dicts[k]['pred_boxes'][selected]
    ...
    def proposal_layer_cp(self, batch_dict, nms_config):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            #TODO: add at 2024-7-2
            if nms_config.get('USE_REG_MASK', False):
                box_select_reverse = pred_dicts_before[idx]['hm'].new_zeros((batch_size, nms_config.TRAIN.NMS_POST_MAXSIZE, final_pred_dicts[0]['pred_boxes'].shape[-1]))
                mask_reg = pred_dicts_before[idx]['hm'].new_zeros((batch_size, nms_config.TRAIN.NMS_POST_MAXSIZE, 1))
                # raw_box_stu = pred_dicts_before[idx]['hm'].new_zeros((batch_size, nms_config.TRAIN.NMS_POST_MAXSIZE, 9 if 'vel' in post_process_cfg.HEAD_ORDER else 8))
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                if nms_config.TRAIN.get('SCORE_THRESH_MINOR'):
                    ...
                    selected, selected_scores = class_agnostic_nms(
                        box_scores=final_dict['pred_scores'], box_preds=final_dict['pred_boxes'],
                        nms_config=nms_config.TRAIN,
                        score_thresh = nms_config.TRAIN.get('SCORE_THRESH', None),
                    )
                ...
                select_index = final_dict['index'][selected]
                cls_select_reverse[k, :len(selected), :] = pred_dicts_before[idx]['hm'][k, :, select_index//width, select_index%width].t()
                # print(final_dict['pred_scores'][selected])
                # print(pred_dicts_before[idx]['hm'][k, :, select_index//width, select_index%width].t().sigmoid().max(-1)[0])

                
                # TODO: add at 2024-7-2
                if nms_config.get('USE_REG_MASK', False):
                    box_select_reverse[k, :len(selected), :] = final_dict['pred_boxes'][selected]
                    iou_stu_tea = iou3d_nms_utils.boxes_iou3d_gpu(final_pred_dicts_tea[k]['pred_boxes'][selected], final_dict['pred_boxes'][selected])
                    iou_stu_tea = torch.diag(iou_stu_tea)
                    # print(iou_stu_tea)
                    mask_reg[k, :len(selected), :] = (iou_stu_tea >= 0.7).unsqueeze(-1)


                    # batch_center_tmp = pred_dict['center'][k, :, select_index//width, select_index%width].t()
                    # batch_center_z_tmp = pred_dict['center_z'][k, :, select_index//width, select_index%width].t()
                    # batch_dim_tmp = pred_dict['dim'][k, :, select_index//width, select_index%width].t()
                    # batch_rot_tmp = pred_dict['rot'][k, :, select_index//width, select_index%width].t()
                    # batch_vel_tmp = pred_dict['vel'][k, :, select_index//width, select_index%width].t() if 'vel' in post_process_cfg.HEAD_ORDER else None

                    # if 'vel' in post_process_cfg.HEAD_ORDER:
                    #     raw_box_stu[k, :len(selected), :] = torch.cat([batch_center_tmp, batch_center_z_tmp, batch_dim_tmp, batch_rot_tmp, batch_vel_tmp], dim=1)
                    # else:
                    #     raw_box_stu[k, :len(selected), :] = torch.cat([batch_center_tmp, batch_center_z_tmp, batch_dim_tmp, batch_rot_tmp], dim=1)
        ...
        if nms_config.get('USE_REG_MASK', False):
            batch_dict['raw_box_stu'] = box_select_reverse
            batch_dict['mask_reg'] = mask_reg
    ...
    def forward(self, batch_dict):
        ...
        if batch_dict.get('batch_cls_preds_tea_densehead'):
            ...
            if self.trans_mode == 'TKD':
                ...
                if self.model_cfg.get('CP2ANCHOR'):
                    _ = self.proposal_layer_cp2anchor(
                        batch_dict, nms_config=self.model_cfg.NMS_CONFIG
                    )
