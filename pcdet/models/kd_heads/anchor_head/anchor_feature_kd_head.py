...
class AnchorFeatureKDHead(KDHeadTemplate):
    ...
    def get_feature_kd_loss_point(self, batch_dict, loss_cfg):
        ...
        select_mask = batch_dict['select_mask'].view(-1, 1) if 'select_mask' in batch_dict else batch_dict['mask_use_item'].view(-1, 1)

        if 'cls_preds_kd_tea' in batch_dict:
            cls_preds_kd_tea = batch_dict['cls_preds_kd_tea']
            cls_preds_kd_stu = batch_dict['cls_preds_kd_stu']
        else:
            cls_preds_kd_tea = batch_dict['cls_preds_selected_reverse_tea']
            cls_preds_kd_stu = batch_dict['cls_preds_selected_reverse_stu']
