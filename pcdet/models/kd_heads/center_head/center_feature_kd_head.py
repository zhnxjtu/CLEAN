...
class CenterFeatureKDHead(KDHeadTemplate):
    ...
    def get_feature_kd_loss_point(self, batch_dict, loss_cfg):
        ...
        # print('aaaaaaaaaaaaaaaaaaa' ,kd_feature_loss)
        
        if 'box_preds_selected_reverse_tea' in batch_dict:
            reg_mask = batch_dict['mask_reg'].view(-1, 1)
            reg_loss = loss_cfg.weight_reverse * \
                            (self.kd_feature_loss_func(batch_dict['box_preds_selected_reverse_tea'] ,
                                                        batch_dict['box_preds_selected_reverse_stu'] ).sum()/ torch.clamp(
                                    reg_mask.sum(), min=1.0))
            
            kd_feature_loss += reg_loss
